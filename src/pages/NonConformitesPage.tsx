...
export default function NonConformitesPage() {
  ...
  // Formulaire de qualification (NC OUVERTE)
  const [qualifyingId, setQualifyingId] = useState<string | null>(null);
  const [causeRacine, setCauseRacine] = useState('');
  const [responsableId, setResponsableId] = useState('');
  const [echeance, setEcheance] = useState('');

  // Formulaire de vérification (NC EN_COURS)
          ...
          <div className="space-y-3">
            {ncList.map((nc) => (
              ...
              >
                ...
                {/* ---------- NC OUVERTE : formulaire de qualification ---------- */}
                {nc.status === 'OUVERTE' && qualifyingId !== nc.$id && (
                  <button
